...
def drop_auto_generated_columns(dataframe):
    ...
    '''
    dataframe = dataframe.drop(columns=misc_constants.SCRIPT_GENERATED_COLUMNS, errors="ignore")
...
def match_column_positions(upload_file_df, db_data_df):
    ...
    if len(upload_file_df.columns) == len(db_data_df.columns):
        ...
        raise Exception(f"Number of columns in database table does match number of columns in upload file. Columns found in database that were not found in upload file: {set(db_data_df.columns) - set(upload_file_df.columns)}. Vice versa: {set(upload_file_df.columns) - set(db_data_df.columns)}")
